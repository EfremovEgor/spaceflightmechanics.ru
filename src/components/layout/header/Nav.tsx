...
const Nav = () => {
        ...
        <Dropdown title={t(`routes.${route.name}`)}>
          <div className="flex flex-col gap-1 px-2 py-4">
            {Object.values(route.routes).map((r) =>
              r.disabled ? (
                <p key={r.name + r.path} className="text-gray-400 text-nowrap">
                  {t(`routes.${r.name}`)}
                </p>
              ) : (
                <Link
  ...
  return (
    <Container className="text-lg py-6 justify-between hidden md:flex">
      {Object.values(Routes).map((v) => (
        <Fragment key={v.name}>{resolveRoute(v)}</Fragment>
      ))}
    </Container>
  );
};
